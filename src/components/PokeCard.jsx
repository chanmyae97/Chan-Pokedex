...
export function PokeCard(props) {
  ...
  try{
    ...
    const description = moveData?.flavor_text_entries.filter(
      val=> {
        return val.version_group.name = 'filtered-leafgreen'
      }
    )[0]?.flavor_text
  ...
  useEffect(() => {
    ...
    //check if  the selected pokemon information is avaliable in the cache.
    // 1. define the cache.
    let cache = {};
    ...
    if (selectedPokemon in cache) {
      ...
      console.log('Founds Pokemon')
      return;
    }

    //we passed all the cahe stuff to no avail and now need to fetch the data from the api
  ...
  return (
    <div className="poke-card">
      ...
      <div className="stats-card">
        {stats.map((statOb, statIndex) => {
          const { stat, base_stat } = statOb;
          return (
      ...
      <div className="pokemon-move-grid">
        {moves.map((moveObj, moveIndex) => {
          return (
            <button
              className="button-cards pokemon-move"
              key={moveIndex}
              onClick={() => {
